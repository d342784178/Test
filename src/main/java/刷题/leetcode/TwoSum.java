...
public class TwoSum {
    ...
    public int[] twoSum2(int[] nums, int target) {
        // 第一次遍历数组先将所有元素和它的下标作为key-value对存入Hashmap中，
        // 第二次遍历数组时根据目标和与当前元素之差，在Hashmap中找相应的差值。如果存在该差值，说明存在两个数之和是目标和。
    ...
    private int[] twoSum3(int[] nums, int target) {
        // 首先将原数组复制一遍，对新数组进行排序。
        // 排序后将双指针指向头部与尾部元素，进行迭代。
        // 如果双指针指向元素之和大于目标和，则将尾部指针向前移一位.
        // 反之则将头部指针向后移一位，直到双指针指向元素之和等于目标和，
        // 记录这两个元素的值，然后再遍历一遍旧数组，找出这两个元素的下标。
        int   left  = 0;
